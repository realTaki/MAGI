...
``magi/runtime/proactive/``):

- ``tasks``        : one row per operator-defined task
- ``task_runs``    : one row per execution attempt (cron or manual)

Why these live in :mod:`magi.runtime.proactive` instead of
:meth:`magi.runtime.state.orm` directly: the proactive
...
:mod:`magi.runtime.state.orm` so the SQLite file is shared
...
in ``_INDEX_MIGRATIONS`` (see :mod:`magi.runtime.state.orm`)
...
- ``id`` is a Crockford ULID (same as ``chat_sessions.session_id``).
  ...
  ``new_ulid()`` from :mod:`magi.runtime.sessions` to keep
  one helper across the codebase.
- Timestamps are ISO UTC strings (``datetime.utcnow().isoformat()``),
  matching the convention in ``magi.runtime.sessions``.
...
from magi.runtime.state.orm import Base
