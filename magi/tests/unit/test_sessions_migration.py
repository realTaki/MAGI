...
``<workspace>/memories/sessions/<delivery_address>/<sid>.json`` tree
...
def fresh_db(monkeypatch, tmp_path):
    ...
    # lives at ``<workspace>/memories/sessions/<delivery_address>/...``.
...
def _write_legacy_session(
    ...
    *,
    ...
    or assert on it.
    """
    sessions_root = workspace / "memories" / "sessions" / delivery_address
...
def test_migrate_imports_active_messages(fresh_db):
    ...
    with open_session() as db:
        ...
        assert sess.delivery_address == "9001"
...
def test_migrate_multiple_tgids(fresh_db):
    """Multiple delivery_address subdirs are walked; sessions are
    imported per chat."""
    from magi.agent.memory.session import migrate_from_json
    from magi.agent.db import ChatSession, open_session
    ...
    assert {r.delivery_address for r in rows} == {"9001", "9002"}
...
def test_migrate_invalid_tgid_dir_is_skipped(fresh_db):
    """A directory whose name violates the delivery_address regex is
    ...
    # outside the delivery_address regex).
