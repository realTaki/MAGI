...
**Adam 和 EVE 是同一个节点。** 共享一个 `magiagent` 包（代理循环、动态上下文、技能运行器、主动引擎、LLM 提供者、审计）和一个进程镜像。每个架构选择都是独立的配置轴——没有任何轴因角色而硬编码：
...
| Adam 对等节点    | `MAGI_ADAM_URL`          | `http://adam:42069`      | 始终读取。任何需要 Adam RPC（审计、配置拉取）的节点都设置此项。 |
