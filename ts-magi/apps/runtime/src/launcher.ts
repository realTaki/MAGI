import { Bus } from "@magi/bus";
import { Caller } from "@magi/caller";
import { DemoProvider } from "@magi/provider";
import { ResultReader } from "@magi/result-reader";
...
export async function launchPlayground(workspace: string): Promise<PlaygroundRuntime> {
  ...
  if (!callerBus || !providerBus || !readerBus) {
    bus.close();
    throw new Error("ts-magi slots are unavailable");
  }
  try {
