...
def test_magi_attaches_default_provider_worker(tmp_path) -> None:
    with Magi(workspace=tmp_path) as magi:
        assert magi.run()
...
def test_magi_workers_share_one_bus(tmp_path) -> None:
    with Magi(
        workspace=tmp_path,
...
def test_magi_rolls_back_when_a_worker_refuses(tmp_path) -> None:
...
def test_magi_rejects_duplicate_worker_id(tmp_path) -> None:
    ...
    with Magi(workspace=tmp_path, worker_types=(One, Two)) as magi:
        with pytest.raises(ValueError, match="duplicate worker_id"):
            magi.run()


def test_asgi_lifespan_attaches_workers_before_serving_api(tmp_path) -> None:
    service = Magi(workspace=tmp_path, worker_types=(SharedLLMWorker,))
